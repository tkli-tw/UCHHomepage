...
loadMainfunction = function(){
	...
	var jqxhr = $.getJSON( "json/mainfunction.json").done(function(data) {
		...
		*/
		...
		var theHolder = document.getElementById('mainoperate');
		...
		$.each(data.items, function(brickCount, item){
			...
			with(brick) {
				...
				style.backgroundColor = BrickMotion.NORMAL_COLOR ;
			...
			flowHolder.place(brick);

			//設定給 _cell 以便 到 Brick.methods() 中做切換
			brick.bindMainPanels = _maindisplay;
			brick.bindShadowPanel = _maindisplay.shadowPanels[brickCount];
			
			// default 為返回 主頁面，需特別處理
			if (item.type === 'default') {    							
				brick.addEventListener('click', BrickMotion.release);
				// default 不需要 對應一個 shadow panel
				brick.bindShadowPanel = null;
			}
			else {
				brick.addEventListener('click', BrickMotion.select);
			}
			
			if (item.outerLink) {
				var innerPanel = document.createElement('iframe');
				innerPanel.src = item.outerLink;
				brick.bindShadowPanel.innerHTML = '';
				brick.bindShadowPanel.appendChild(innerPanel);
				brick.bindShadowPanel.innerframe = innerPanel;  //建立一個 連結，指向這個 iframe，供 Brick.select() 中使用
				with (innerPanel.style) {
					position = 'absolute';
					left = '0px';
					top = '0px';
					width = brick.bindShadowPanel.style.width;
					height = brick.bindShadowPanel.style.height;
				}
			}
			...
			if (item.type === 'extendLinkSet'){
				...
				with (brick.bindShadowPanel.style) {
				...
				brick.bindShadowPanel.appendChild(descriptionPanel);
				brick.bindShadowPanel.appendChild(linkPanel);
				...
				with(descriptionPanel.style){
					width = parseInt(brick.bindShadowPanel.style.width) - parseInt(linkPanel.style.width) + 'px';
					height = brick.bindShadowPanel.style.height;
